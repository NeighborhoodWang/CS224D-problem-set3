...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            node.hActs1 = W.dot(np.hstack((node.left.hActs1, node.right.hActs1))) + b
            node.hActs1[node.hActs1 < 0] = 0
        
        x = Ws.dot(node.hActs1) + bs
        x -= np.max(x)
    ...
    def backProp(self,node,error=None):
        ...
        self.dWs += np.outer(error_this, node.hActs1)
        self.dbs += error_this
        ...
        if node.isLeaf:
            ...
            self.dW += np.outer(delta, np.hstack([node.left.hActs1, node.right.hActs1]))
            self.db += delta
